...
export const EditBill: FC<{
	billNumber: string
	defaultValue: string
}> = ({ billNumber, defaultValue }) => {
	const [desc, setDesc] = useState(defaultValue)

	const queryClient = useQueryClient()

	const { mutate: editBill } = useMutation({
		mutationKey: ['edit bill desc', billNumber],
		mutationFn: ({ description }: { description: string }) =>
			InvoicesService.editBill({ billNumber, description }),
		onSuccess: () => {
			...
			<PopoverContent className="w-80 bg-white">
				<div className="grid gap-4">
					<div className="space-y-2">
						...
						<p className="text-muted-foreground text-sm">Edit bill</p>
					</div>
